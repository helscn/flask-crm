<template>
  ...
  >
    <div>
      <div class="text-subtitle1">开始时间：</div>
      <q-input filled v-model="start_date" class="col">
        ...
        <template v-slot:append>
          <q-icon name="access_time" class="cursor-pointer">
            <q-popup-proxy transition-show="scale" transition-hide="scale">
              <q-time v-model="start_date" mask="YYYY-MM-DD HH:mm:ss" format24h>
                <div class="row items-center justify-end">
                  <q-btn v-close-popup label="Close" color="primary" flat />
                </div>
              </q-time>
    ...
    <div>
      <div class="text-subtitle1">结束时间：</div>
      <q-input filled v-model="end_date" class="col">
        <q-tooltip :delay="400" anchor="top middle">
    ...
    <div class="row justify-between">
      <q-input v-model="weeks" label="星期间隔" class="col q-mr-md">
        <q-tooltip :delay="400" anchor="top middle">
          需要运行的计划任务引用，任务模块必须放在服务器 jobs 文件夹中，格式：
          模块名:入口函数名
        </q-tooltip>
      </q-input>
      <q-input v-model="days" label="天数间隔" class="col">
        <q-tooltip :delay="400" anchor="top middle">
          需要运行的计划任务引用，任务模块必须放在服务器 jobs 文件夹中，格式：
          模块名:入口函数名
        </q-tooltip>
    ...
    <div class="row justify-between">
      <q-input v-model="hours" label="小时间隔" class="col q-mr-md">
        <q-tooltip :delay="400" anchor="top middle">
          需要运行的计划任务引用，任务模块必须放在服务器 jobs 文件夹中，格式：
          模块名:入口函数名
        </q-tooltip>
      </q-input>
      <q-input v-model="minutes" label="分钟间隔" class="col">
        <q-tooltip :delay="400" anchor="top middle">
          需要运行的计划任务引用，任务模块必须放在服务器 jobs 文件夹中，格式：
          模块名:入口函数名
        </q-tooltip>
